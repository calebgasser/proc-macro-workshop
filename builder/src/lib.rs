...
pub fn derive_builder(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let impl_fields = match get_impl_fields(&input.data) {
        Ok(i) => i,
        Err(e) => return e.into_compile_error().into(),
    };
    let impl_funcs = match get_impl_funcs(&input.data) {
        Ok(i) => i,
        Err(e) => return e.into_compile_error().into(),
    };
...
fn attr_builder_value(attrs: &Vec<Attribute>) -> syn::Result<Option<BuilderParser>> {
    for attr in attrs {
        if attr.path().is_ident("builder") {
            let builder: BuilderParser = attr.parse_args()?;
...
fn get_impl_funcs(data: &Data) -> syn::Result<TokenStream> {
    match *data {
        Data::Struct(ref data) => match data.fields {
            Fields::Named(ref fields) => {
                ...
                for f in fields.named.iter() {
                    if let Some(builder) = attr_builder_value(&f.attrs)? {
                        recurse.push(impl_func_builder(&builder, &f));
                    } else {
                        recurse.push(impl_func(&f));
                    }
                }
                Ok(quote! {
                    #(#recurse)*
...
fn get_impl_fields(data: &Data) -> syn::Result<TokenStream> {
    match *data {
        Data::Struct(ref data) => match data.fields {
            Fields::Named(ref fields) => {
                let mut recurse = Vec::new();
                for f in fields.named.iter() {
                    let name = &f.ident;
                    if let Some(_builder) = attr_builder_value(&f.attrs)? {
                        recurse.push(quote_spanned! { f.span()=>
                            #name: Some(Vec::new()),
                        });
                    } else {
                        recurse.push(quote_spanned! { f.span()=>
